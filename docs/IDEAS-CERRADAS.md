# Ideas cerradas — sdd-first

> Archivo de los ítems de [`IDEAS.md`](IDEAS.md) que ya se cerraron, **con su
> post-mortem íntegro**: qué encontró la implementación que la idea no
> registraba, y qué se descartó por el camino. Es la parte más cara de
> reconstruir y por eso no se borra; se saca del backlog para que el backlog
> siga siendo legible.
>
> Los IDs son estables y no se renumeran (los citan specs, historial y código).
> La convención de IDs, los estados y las prioridades están en
> [`IDEAS.md`](IDEAS.md); los patrones que se repiten entre estos ítems, en
> [`PATRONES.md`](PATRONES.md).
>
> Las secciones conservan la tanda de origen, que es el contexto que le da
> sentido a cada hallazgo. Dentro de cada una faltan los ítems que siguen
> abiertos: esos viven en [`IDEAS.md`](IDEAS.md).

## Qué idea terminó en qué spec

| ID | Ítem | Cerrado por |
|----|------|-------------|
| D-1..D-4 | Dogfooding roto: gate, tests, índice y SPEC-001 del propio kit | [[SPEC-002-dogfooding-integro]] |
| B-1..B-4 | Happy path de instalación roto (4 bugs reproducidos) | [[SPEC-003-install-happy-path]] |
| G-1 | Pre-commit hardcodea `files: '^(src\|app\|lib)/'` | [[SPEC-015-wiring-apunta-al-codigo-real]] |
| G-2 | El gate no verifica el estado de la spec | [[SPEC-006-gate-verifica-estado-spec]] |
...
| V-4 | La raíz de `tests/` no la mira ningún paso | [[SPEC-019-tests-integracion-ejecutados]] |
| E-1 | El proyecto instalado no recibe las skills `sdd-*` | [[SPEC-007-derived-project-onboarding]] · [[SPEC-016-skills-listas-tras-init]] |
...
| F-2 | Sin CI, ni propia ni instalable | [[SPEC-009-coverage-y-ci]] |
| F-3 | `gen_skill_adapters.py` sin documentación | [[SPEC-010-gobernanza-y-docs]] |
| F-4 | La constitución generada era más pobre que la de referencia | [[SPEC-010-gobernanza-y-docs]] |
| F-5 | E-6 era más ancho de lo registrado (ocho plantillas) | [[SPEC-010-gobernanza-y-docs]] |
| F-6 | `.gitattributes` no forzaba LF en los `.sh` | — (corregido en el repo) |
| F-7 | Módulos del núcleo con 0% de cobertura | *superseded* por K-3 |
| U-1..U-3 | Config sembrado con layout ajeno; `dirs` sin documentar; omitido = OK | [[SPEC-003-install-happy-path]] |
| U-4..U-11 | El derivado reporta salud sin haberla medido, y habla en términos del kit | [[SPEC-014-derivado-dice-la-verdad]] |
| K-1 | El derivado nace sin el paso `render` | [[SPEC-014-derivado-dice-la-verdad]] |
| K-2 | El catálogo de claves del config no viaja con el derivado | [[SPEC-013-proyecto-derivado-coherente]] |
| K-3 | La cobertura del kit está mal calibrada | Principio V (75% → 91%) |
