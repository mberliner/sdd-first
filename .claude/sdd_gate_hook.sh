...
# (SSOT agnostico de asistente). Contrato: exit 0 = permitir, exit 2 = bloquear
# (stderr lleva el motivo).
#
# FAIL-CLOSED: si ningun interprete logra correr el gate, se BLOQUEA la
# edicion bajo core/ o adapters/ (los source_roots del propio kit, ver
# .sdd/config.yaml) en vez de permitirla en silencio. Fuera de esas rutas se
...
if [ -z "$PYBIN" ]; then
  case "$INPUT" in
    *'"core/'* | */core/* | *'"adapters/'* | */adapters/*)
      ...
      echo "Se BLOQUEA la edicion bajo core/ o adapters/ (fail-closed). Crea el .venv del proyecto o instala python3." >&2
      exit 2
      ;;
  esac
  exit 0
